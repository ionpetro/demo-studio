...
- The flow pays for anything or changes/destroys data — the recording agent
...
1. Call `create_loopa` with:
   - `goal` — one or two sentences describing what the loopa should show.
     Never put credentials in the goal.
...
4. Pages behind a login: if the user previously signed in to that site through
   ...
   it to the user and ask them to open it in their own browser and sign in —
   never open it or enter credentials yourself. Then call `confirm_login` with
   the `runId` to resume (some login handoffs resume automatically; calling it
   anyway is harmless). The run gives up after 10 minutes at a login wall.
5. While recording, `liveViewUrl` lets a human watch the browser live.
6. Sites behind aggressive bot protection (e.g. Cloudflare challenges) may
